...
export const updateProfile = async (req:Request, res:any) => {
    try {
        ...
        if(!username) {
            return  res.status(400).json({message:"username does not exists", status:400, success:false})

        }
        const user = await User.findOne({username})

        if(!user) {
             return  res.status(400).json({message:"user does not exists", status:400, success:false})
        }
      ...
      user: {
        ...
        res.status(500).json({message:"Internal server error", status:500, success:true})
    }
}